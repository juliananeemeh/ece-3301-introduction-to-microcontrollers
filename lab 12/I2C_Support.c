...
int DS1621_Read_Temp()                                                          // function to read temp
{
  char Device = 0x48;
  char Cmd = READ_TEMP;
  char Data_Ret;    
...
int DS1621_Read_Temp_Bad()                                                      // function to read bad temp
{
  char Device = 0x49;
  char Cmd = READ_TEMP;
  char Data_Ret;    
...
void DS3231_Read_Time()                                                         // function to read time
{
  ...
  I2C_ReStart();                                                                // Restart I2C
  I2C_Write((Device << 1) | 1);                                                 // Initialize data read         
  second = I2C_Read(ACK);                                                       // read from second, minute, hour, dow, day, month, year
  minute = I2C_Read(ACK);
  hour = I2C_Read(ACK);
  dow = I2C_Read(ACK);
  day = I2C_Read(ACK);
...
void DS3231_Setup_Time()                                                        // function to setup time
{
  char Device = 0x68;
  char Address = 0x00;
  second = 0x00;                                                                // initialize values for second, minute, hour, dow, day, month, year
  minute = 0x30;
  hour = 0x02;
  dow = 0x05;
  day = 0x30;
