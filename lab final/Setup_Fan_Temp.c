...
void Setup_Fan_Temp(void)
{
    ...
    {   
        ...
        if (check_for_button_input() == 1)
        {
             ...
             {
                Do_Beep_Good(); 
                if (found == Play_Pause) Do_Save_New_Fan_Temp();                // if found is play/pause, call do save new fan temp function
                if (found == EQ) Exit_Setup_Fan_Temp();                         // if found is EQ, call exit setup fan temp function
                if (found == Plus) Increase_Fan_Temp();                         // if found is Plus, call increase fan temp function
                if (found == Minus) Decrease_Fan_Temp();                        // if found is minus, call decrease fan temp function
...
void  Increase_Fan_Temp()		// routine for 'Plus' key
{
    ...
    setup_fan_set_temp = setup_fan_set_temp + 1;                                // increment setup fan set temp
    if (setup_fan_set_temp > 110)                                               // if setup fan set temp greater than 110, set to 110
    {
        setup_fan_set_temp = 110;
    };
...
void  Decrease_Fan_Temp()                                                       // routine for 'Minus' key     
{
    ...
    setup_fan_set_temp = setup_fan_set_temp - 1;                                // decrement setup fan set temp
    if (setup_fan_set_temp < 50)                                                // if setup fan set temp less than 50, set to 50
    {
...
void Update_Setup_Fan_Screen(void)
{
    ...
    char dc_char2 = ((setup_fan_set_temp / 10) % 10);                           // add code here temp ten 								
