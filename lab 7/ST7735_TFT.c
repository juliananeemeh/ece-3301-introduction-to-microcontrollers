...
#define COUNT_SCALED    COUNT_PER_MS/8     // 
	...
	TMR3L = count & 0x00ff;             // set the lower byte of TMR
	TMR3H = count >> 8;                 // set the upper byte of TMR
...
void Bcmd()
{
  write_command(ST7735_SWRESET);        // Write command 0x01
  delay_ms(50);
  write_command(ST7735_SLPOUT);         // Write command 0x11
  delay_ms(500);
...
void Rcmd2red()
{
  write_command(ST7735_CASET);              // Write command 0x2a
  write_data(0x00);                         // Write data 0x00
  write_data(0x00);                         // Write data 0x00
  write_data(0x00);                         // Write data 0x00
  write_data(0x7F);                         // Write data 0x7F
  write_command(ST7735_RASET);              // Write command 0x2b
  write_data(0x00);                         // Write data 0x00
  write_data(0x00);                         // Write data 0x00
  write_data(0x00);                         // Write data 0x00
  write_data(0x9F);                         // Write data 0x9F
}
