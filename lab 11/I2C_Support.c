...
{
  char Device = 0x48;               // set device hex code
  char Cmd = READ_TEMP;
  char Data_Ret;    
...
int DS1621_Read_Temp_Bad()          // function to read temperature with incorrect device code
{
  char Device = 0x49;               // incorrect hex code (bad temp)
  char Cmd = READ_TEMP;
  char Data_Ret;    
  I2C_Start();                      // Start I2C protocol
  I2C_Write((Device << 1) | 0);     // Device address
...
void DS1621_Init()                  // function to initialize device
{
    char Device = 0x48;
...
void DS3231_Read_Time()             // function to read time using I2C
{
  char Device = 0x68;               // initialize device code
  char Address = 0x00;
  char Data_Ret;    
  I2C_Start();                      // Start I2C protocol
  ...
  I2C_ReStart();                    // Restart I2C
  I2C_Write((Device << 1) | 1);     // Initialize data read         
  second = I2C_Read(ACK);           // read timing information 
  minute = I2C_Read(ACK);
  hour = I2C_Read(ACK);
  dow = I2C_Read(ACK);
  day = I2C_Read(ACK);
...
void DS3231_Setup_Time()          // function to set time using I2C
{
  char Device = 0x68;
  char Address = 0x00;
  second = 0x00;                    // initialize information
  minute = 0x30;
  hour = 0x02;
  dow = 0x05;
  day = 0x30;
