...
void Initialize_LCD_Screen()
{
  ...
  strcpy(txt, "EWP EWLT NSP NSLT R S");
  drawtext(1,  Switch_Txt_Y, txt, ST7735_WHITE, ST7735_BLACK, TS_1);
  drawtext(8,  Switch_Txt_Y+9, SW_EWPED_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
  drawtext(35, Switch_Txt_Y+9, SW_EWLT_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
  drawtext(60, Switch_Txt_Y+9, SW_NSPED_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
  drawtext(85, Switch_Txt_Y+9, SW_NSLT_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
  ...
  Timing_Txt[10] = PEDESTRIAN_EW_WAIT + '0';
  Timing_Txt[11] = EW_WAIT + '0';
  Timing_Txt[12] = NS_LT_WAIT + '0';
  Timing_Txt[13] = PEDESTRIAN_NS_WAIT + '0';
  Timing_Txt[14] = NS_WAIT + '0';
  Timing_Txt[15] = EW_LT_WAIT + '0';

  Timing_Txt[17] = NIGHT_EW_WAIT + '0';
  Timing_Txt[18] = NIGHT_NS_LT_WAIT + '0';
  Timing_Txt[19] = NIGHT_NS_WAIT + '0';
  Timing_Txt[20] = NIGHT_EW_LT_WAIT + '0';
...
void update_LCD_color(char direction, char color)
{
    ...
    else if (color == Color_Red)     //if Color red make all circles red but leave outline
    {
    ...
    else if (color == Color_Yellow)     //if Color yellow make all circles yellow but leave outline
    {
        fillCircle(XRED, Circle_Y, Circle_Size, ST7735_BLACK);
...
void update_LCD_PED_Count(char direction, char count)
{
   switch (direction)
   {
      case 0:       
		...
		PORTD = (PORTD & mask_TFT) | (count << 0);                                      // output count leds
		DP = 0x00;                                                                      // Set DP		
        drawtext(PED_Count_X, PED_EW_Count_Y, PED_EW_Count, EW_Color, ST7735_BLACK, TS_2);        
        break;
	  case 1:       
        PED_NS_Count[0] = count/10  + '0';                                              // PED count lower digit
...
void update_LCD_misc()
{
    ...
    SW_NSLT = NS_LT_SW;
	 ...
	 // put code here for SW_EWLT, SW_NSPED, SW_NSLT,
    if (SW_EWPED == 0) SW_EWPED_Txt[0] = '0'; else SW_EWPED_Txt[0] = '1';               // text for EWPED, EWLT, NSPED, NSLT switches
    ...
    drawtext(8,  Switch_Txt_Y+9, SW_EWPED_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
    drawtext(35, Switch_Txt_Y+9, SW_EWLT_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
    drawtext(60, Switch_Txt_Y+9, SW_NSPED_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);       // Show switch and sensor states at bottom of the screen
    drawtext(85, Switch_Txt_Y+9, SW_NSLT_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
    drawtext(108, Switch_Txt_Y+9, FlashingR_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);  
    drawtext(119, Switch_Txt_Y+9, FlashingS_Txt, ST7735_WHITE, ST7735_BLACK, TS_1); 
